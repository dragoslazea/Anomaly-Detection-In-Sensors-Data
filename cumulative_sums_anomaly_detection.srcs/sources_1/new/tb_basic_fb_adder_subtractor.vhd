...
entity tb_basic_fp_adder_subtractor is
end tb_basic_fp_adder_subtractor;

architecture Behavioral of tb_basic_fp_adder_subtractor is
component fifo
  Port (
...
component fp_adder_subtracter is
  Port ( 
    aclk : IN STD_LOGIC;
...
signal f1_out_data, f2_out_data, f3_in_data, f3_out_data : STD_LOGIC_VECTOR (31 downto 0) := (others => '0');
signal f1_in_data : STD_LOGIC_VECTOR (31 downto 0) := "01000001101000000000000000000000"; -- 20.0
signal f2_in_data : STD_LOGIC_VECTOR (31 downto 0) := "01000001001000000000000000000000"; -- 10.0
...
signal f1_rdc, f2_rdc, f3_rdc : STD_LOGIC_VECTOR (31 downto 0) := (others => '0');
signal f1_wdc, f2_wdc, f3_wdc : STD_LOGIC_VECTOR (31 downto 0) := (others => '0');
signal f1_dc, f2_dc, f3_dc : STD_LOGIC_VECTOR (31 downto 0) := (others => '0');
...
begin
    ...
    f1 : fifo port map (
        ...
        axis_data_count => f1_dc,
        axis_wr_data_count => f1_wdc,
        axis_rd_data_count => f1_rdc 
    );
    ...
    f2 : fifo port map (
        ...
        axis_data_count => f2_dc,
        axis_wr_data_count => f2_wdc,
        axis_rd_data_count => f2_rdc 
    );
    
    add_input_a_valid <= f1_out_valid and s;
    add_input_b_valid <= f2_out_valid and p;
    
    add_sub : fp_adder_subtracter port map (
        aclk => clk,
    ...
    f3 : fifo port map (
        ...
        axis_data_count => f3_dc,
        axis_wr_data_count => f3_wdc,
        axis_rd_data_count => f3_rdc 
    );
    ...
    begin
        if rising_edge(clk) then
            if f2_in_ready = '1' then
                f2_in_data <= f2_in_data + 1;
            end if;
        end if;
    end process;
    
    -- f1_in_data <= x"00000034"; -- 52
    
    clk <= not clk after T / 2;
    nrst <= '0', '1' after 5 * T;
    
    f1_in_valid <= '0', '1' after 10 * T;
