...
entity basys3_cusum_anomaly_detector is
...
end basys3_cusum_anomaly_detector;

architecture Structural of basys3_cusum_anomaly_detector is
...
signal next_data_en, mpg_en, abnormal, nrst, next_en : STD_LOGIC := '0';
...
signal measurement_index, current_measurement, previous_measurement, timestamp, result_count, anomaly_count : STD_LOGIC_VECTOR (31 downto 0) := (others => '0');
...
begin
    ...
    monopulse_generator : mpg port map (clk => clk, btn => btn(0), en => mpg_en);
    
    --data_counter : counter32 port map (clk => clk, clr => btn(1), en => mpg_en, q => measurement_index);
    
    measurements_memory : fp_rom256x32 port map (address => measurement_index, data_out_previous => previous_measurement, data_out_current => current_measurement);
        ...
        if rising_edge(clk) then
            if nrst = '1' then
                if current_ready = '1' and previous_ready = '1' then
                    --if measurement_index < 254 then
                    if current_measurement /= x"00000000" then
    ...
    cusum_anomaly_detector : cumulative_sums_detector port map (
        clk => clk,
        rst => nrst,
    ...
    to_display <= current_measurement (15 downto 0) when sw(2 downto 0) = "000" else
                  ...
                  --timestamp (15 downto 0) when sw(2 downto 0) = "100" else
                  anomaly_count (15 downto 0) when sw(2 downto 0) = "100" else
                  --timestamp (31 downto 16) when sw(2 downto 0) = "101" else
